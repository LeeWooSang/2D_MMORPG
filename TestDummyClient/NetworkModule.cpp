...
constexpr int MAX_HOTSPOT_COUNT = 300;
constexpr int MAX_CCU_COUNT = 1000;
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_LOGIN_OK:
	{
		...
		{
			g_clients[login_packet->id].connect = true;
	...
	case SC_ADD_OBJECT: 
	case	SC_WEAPON_INVENTORY:
	case	SC_POTION_INVENTORY:
	case SC_PICKUP_WEAPON:
	case SC_PICKUP_POTION:
	case SC_ATTACK:
	case SC_GETDAMAGE:
	case SC_GETEXP:
	case SC_ITEMDROP:
	case SC_REMOVE_ITEM:
	case SC_REMOVE_INVENTORY_ITEM:
	case SC_USE_POTION_ITEM:
	case SC_EQUIP_WEAPON_ITEM:
	case SC_HP_HEAL:
		break;
...
{
	if (g_TestType == HOTSPOT)
	{
		if (num_connections >= MAX_HOTSPOT_COUNT)
			return;
	}
	else if(g_TestType == CCU)
	{
		if (num_connections >= MAX_CCU_COUNT)
			return;
	}

	if (high_resolution_clock::now() < last_connect_time + 100ms) 
		return;
	...
	int ret = WSARecv(g_clients[num_connections].client_socket, &g_clients[num_connections].recv_over.wsabuf, 1, NULL, &recv_flag, &g_clients[num_connections].recv_over.over, NULL);
	...
	Send_DummyClient_Login_Packet(num_connections);
...
void SendPacket(int cl, void *packet)
{
	...
	int ptestType = reinterpret_cast<unsigned char *>(packet)[2];
	...
	while (true) 
	{
		...
		for (int i = 0; i < num_connections; ++i) 
		{
			if (false == g_clients[i].connect) 
				continue;
			if (g_clients[i].x < 50 && g_clients[i].y < 50)	continue;
...
{
	cout << "[ Stress Test Server Type]" << endl;
	cout << "1. 동접 테스트" << endl;
	cout << "2. HotSpot 테스트" << endl;
	cout << "입력하세요 : ";
	cin >> g_TestType;
	switch (g_TestType)
	{
...
void Send_DummyClient_Login_Packet(int id)
{
	DCS_Packet_Login my_packet;
	my_packet.size = sizeof(DCS_Packet_Login);

	my_packet.type = DCS_LOGIN;
	my_packet.testType = g_TestType;
